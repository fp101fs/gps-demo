...
import { View, Text, ScrollView, Alert, Platform, Share, Switch, Image, TextInput, TouchableOpacity, Modal, useWindowDimensions } from 'react-native';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const checkAndWelcome = async () => {
          ...
          if (count === 0) {
              await Notifications.send(user.id, 'Welcome!', 'Start tracking your journey to get started.', 'success');
          }
      };
      checkAndWelcome();
    }
      ...
      adHocMembers.forEach(async (member) => {
          ...
          if (!isNaN(arrThreshold) && arrivalEnabled) {
              if (dist <= arrThreshold && !arrivedMembersRef.current.has(member.id)) {
                  ...
                  await Notifications.send(user!.id, 'Arrival Alert!', `A member has arrived (${Math.round(dist)}m away).`, 'success');
  ...
  }, [currentPoint, adHocMembers, proximityEnabled, proximityDistance, arrivalEnabled, arrivalDistance]);
  ...
  const fetchAddress = async (lat: number, lng: number) => {
    try {
      ...
      setAddress(addr ? `${addr.road || ''} ${addr.house_number || ''}, ${addr.city || ''}`.trim().replace(/^,/, '') : data.display_name || 'Unknown');
    } catch (e) {}
  };
        ...
        <View className="flex-1 justify-center items-center bg-black/50 p-6">
            <View className="bg-white dark:bg-gray-900 p-8 rounded-3xl items-center shadow-xl w-full max-w-sm">
                ...
                <View className="bg-white p-4 rounded-2xl mb-6 shadow-sm border border-gray-100">
                    <QRCode value={trackId ? `${Platform.OS === 'web' ? window.location.origin : Linking.createURL('/')}/track/${trackId}` : 'https://gps-demo.vercel.app'} size={200} color="black" backgroundColor="white" />
                </View>
